...
import { ASPECTS, PLANETS, type NatalChart, type PlanetPosition, type Transit, type TransitData, type TransitResponse } from './types.js';
  ...
  for (const t of transits) {
    ...
    if (!existing) {
      ...
      const shouldReplace =
        ...
        (t.orb === existing.orb && t.exactTime && existing.exactTime && t.exactTime < existing.exactTime) ||
        (t.orb === existing.orb && !t.exactTime && !existing.exactTime && t.transitLongitude < existing.transitLongitude) ||
        (t.orb === existing.orb && !t.exactTime && !existing.exactTime && t.transitLongitude === existing.transitLongitude && t.natalLongitude < existing.natalLongitude) ||
        (t.orb === existing.orb && !t.exactTime && !existing.exactTime && t.transitLongitude === existing.transitLongitude && t.natalLongitude === existing.natalLongitude && t.transitingPlanet < existing.transitingPlanet) ||
        (t.orb === existing.orb && !t.exactTime && !existing.exactTime && t.transitLongitude === existing.transitLongitude && t.natalLongitude === existing.natalLongitude && t.transitingPlanet === existing.transitingPlanet && t.natalPlanet < existing.natalPlanet);
...
export class TransitCalculator {
  ...
  ): { exactTime?: Date; status: NonNullable<Transit['exactTimeStatus']>; selectedRoot: number | null } {
