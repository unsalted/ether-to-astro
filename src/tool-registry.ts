...
export const MCP_TOOL_SPECS: ToolSpec[] = [
  ...
  },
  {
    name: 'get_transits',
    description:
      'Get transits (aspects between current/future planets and natal chart). Supports mode=snapshot (single-day), mode=best_hit (multi-day best-hit compression), and mode=forecast (day-by-day, non-compressed). Date defaults to today at local noon in the natal chart timezone.',
    inputSchema: {
      type: 'object',
      properties: {
        date: {
        ...
        days_ahead: {
          type: 'number',
          description:
            'Number of days to look ahead. In best_hit/forecast modes this is a range length; in snapshot mode only the start day is used. 0 = today only. Defaults to 0.',
          default: 0,
        },
        mode: {
          type: 'string',
